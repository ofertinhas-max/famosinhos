...
function getApiKey() {
  try {
    const config = db.get('SELECT valor FROM configuracoes WHERE chave = ?', ['escalapay_api_key']);
...
function getBaseUrl() {
  try {
    const config = db.get('SELECT valor FROM configuracoes WHERE chave = ?', ['escalapay_base_url']);
    if (config && config.valor && config.valor.trim()) {
      return config.valor.trim();
    }
  } catch (err) {
...
function getBaseUrlCreate() {
  try {
    const config = db.get('SELECT valor FROM configuracoes WHERE chave = ?', ['escalapay_base_url_create']);
    if (config && config.valor && config.valor.trim()) {
      return config.valor.trim();
    }
  } catch (err) {
...
export async function createPixTransaction({
  ...
  if (!apiKey) {
    throw new Error('API Key do EscalaPay não configurada. Configure em Configurações > Gateway de Pagamento');
  }
  ...
  try {
    ...
    const response = await fetch(`${baseUrlCreate}/functions/v1/transactions-create`, {
      method: 'POST',
      headers: {
    ...
    if (!response.ok) {
      ...
      if (response.status === 400) {
        throw new Error('Dados inválidos enviados ao gateway. Verifique os campos obrigatórios.');
      } else if (response.status === 401) {
        throw new Error('API Key inválida ou ausente. Verifique as configurações do gateway.');
      } else if (response.status === 403) {
        throw new Error('Conta inativa no gateway. Entre em contato com o suporte.');
      } else if (response.status >= 500) {
        throw new Error('Erro interno do gateway. Tente novamente em alguns instantes.');
      } else {
    ...
    const provedor_response = data.provedor_response || data.provedorResponse || {};
    if (!sucesso && !transacao.transaction_id) {
      console.error('[EscalaPay] ❌ Resposta inválida do gateway:', data);
      throw new Error('Resposta inválida do gateway de pagamento');
    }
    
    const transaction_id = transacao.transaction_id || provedor_response.transaction_id || data.transaction_id;
    const qrcode = provedor_response.qrcode || provedor_response.qr_code
      || transacao.pix_copia_cola || transacao.qr_code;
    if (!qrcode) {
      console.error('[EscalaPay] ❌ Dados essenciais faltando na resposta:', data);
    ...
    if (pedidoId && transaction_id) {
      try {
        ...
        console.log('[EscalaPay] 💾 Transação vinculada ao pedido #' + pedidoId);
      } catch (dbErr) {
        console.error('[EscalaPay] ⚠️ Erro ao salvar transação no banco (não crítico):', dbErr);
      }
    } else {
      console.log('[EscalaPay] ℹ️ Transação criada sem pedido_id (será vinculado posteriormente)');
    }
    
    const qr_code_image = provedor_response.qrcode_image || provedor_response.qr_code || transacao.qr_code || null;
    return {
      success: true,
    ...
    if (error.name === 'TypeError' || error.name === 'FetchError' || error.name === 'AbortError' || error.code === 'ECONNREFUSED' || error.code === 'ENOTFOUND' || error.code === 'ETIMEDOUT') {
      console.error('[EscalaPay] ❌ Erro de conexão com o gateway:', { message: msg, code: error?.code, cause, baseUrl: getBaseUrlCreate() });
      throw new Error('Erro ao conectar com o gateway de pagamento. Verifique sua conexão. Detalhe: ' + (error?.code || msg));
    }
...
export async function processWebhook(webhookData) {
  ...
  const { transaction_id, status, valor, descricao, data_confirmacao } = webhookData;
  ...
  const tx = db.get('SELECT * FROM transacoes_pix WHERE transaction_id = ?', [transaction_id]);
  
  if (!tx) {
    // Tentar buscar pedido diretamente pelo pix_payment_id
    const pedido = db.get('SELECT * FROM pedidos WHERE pix_payment_id = ?', [transaction_id]);
    
    if (!pedido) {
      console.error('[EscalaPay Webhook] ❌ Transação e pedido não encontrados:', transaction_id);
    ...
    if (status === 'confirmado') {
      console.log('[EscalaPay Webhook] ✅ PAGAMENTO CONFIRMADO - Atualizando pedido');
      ...
      console.log('[EscalaPay Webhook] 💰 Pedido #' + pedido.id + ' marcado como PAGO');
      ...
      sendTikTokConversion(db, {
        ...
        trackingData: {
          event_source_url: pedido.url_origem || `${process.env.SITE_URL}/checkout`,
        ...
        console.log('[EscalaPay Webhook] 🎯 TikTok conversion results:', results);
      }).catch(err => {
        console.error('[EscalaPay Webhook] ❌ Erro ao enviar conversão TikTok:', err);
      });
  ...
  if (!pedido) {
    console.error('[EscalaPay Webhook] ❌ Pedido não encontrado para transação:', tx.pedido_id);
  ...
  if (status === 'confirmado') {
    console.log('[EscalaPay Webhook] ✅ PAGAMENTO CONFIRMADO - Atualizando pedido e transação');
...
function getUpsellRedirectUrl(pedido) {
  try {
    ...
    const incluirParams = db.get('SELECT valor FROM taxas_links WHERE chave = ?', ['incluirQueryParams']);
    const incluirQueryParams = incluirParams && incluirParams.valor === '1';
    
    // Buscar taxas ativas (taxa1 a taxa5)
    for (let i = 1; i <= 5; i++) {
      const taxaKey = `taxa${i}`;
      const taxaConfig = db.get('SELECT valor FROM taxas_links WHERE chave = ?', [taxaKey]);
        ...
        try {
          ...
          if (taxa.ativo && taxa.url && taxa.url.trim()) {
            ...
            if (incluirQueryParams) {
              ...
              params.append('documento', (pedido.cliente_cpf || '').replace(/\D/g, ''));
              params.append('telefone', (pedido.cliente_telefone || '').replace(/\D/g, ''));
            ...
            console.log('[EscalaPay] 🎯 Upsell ativo encontrado:', { taxa: taxaKey, url });
            return url;
          }
        } catch (err) {
          console.warn('[EscalaPay] Erro ao parsear taxa ' + taxaKey + ':', err);
        }
      }
    }
